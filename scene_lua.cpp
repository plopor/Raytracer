...
int gr_nh_sphere_cmd(lua_State* L)
{
  ...
  bool tex = luaL_checknumber(L, 4);
  bool bum = luaL_checknumber(L, 5);

  data->node = new GeometryNode(name, new NonhierSphere(pos, radius, tex, bum));
...
int gr_render_cmd(lua_State* L)
{
  ...
  for (int i = 1; i <= light_count; i++) {
	...
	RT_Render(root->node, im, eye, view, up, fov, ambient, lights);
    im.savePng( filename );
...
int gr_material_cmd(lua_State* L)
{
  ...
  double index = luaL_checknumber(L, 4);
  double mirror = luaL_checknumber(L, 5);
  bool glossy = (bool)luaL_checknumber(L, 6);
